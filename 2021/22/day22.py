...
def all_reboot(instr):
    ...
    for i in instr:
        ...
        for (ex0, ex1, ey0, ey1, ez0, ez1), esgn in cuboid.items():
            ix0 = max(ax, ex0); ix1 = min(ay, ex1)
            iy0 = max(bx, ey0); iy1 = min(by, ey1)
            iz0 = max(cx, ez0); iz1 = min(cy, ez1)
...
print(sum((ay - ax + 1) * (by - bx + 1) * (cy - cx + 1) * sgn for (ax, ay, bx, by, cx, cy), sgn in cubes.items()))
